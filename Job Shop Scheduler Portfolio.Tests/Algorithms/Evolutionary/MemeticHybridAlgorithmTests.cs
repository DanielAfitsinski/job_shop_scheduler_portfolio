...
public class MemeticHybridAlgorithmTests
{
	[Fact]
	public void Metadata_ReturnsExpectedValues()
	{
		// Arrange
		var algorithm = new MemeticHybridAlgorithm();

		// Act & Assert
		Assert.Equal(AlgorithmId.MemeticHybrid, algorithm.Id);
		Assert.Equal("Memetic Hybrid", algorithm.DisplayName);
